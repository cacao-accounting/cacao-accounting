...
- **Fix issues #119/#145 - S2P-01/O2C-09: Verificación de relaciones activas al cancelar documentos (2026-07-09):** Se agregó `has_active_source_relations()` a todos los handlers de cancelación en Compras (PR, SQ, PQ, Receipt) y Ventas (SR, SQ, DN, Invoice) que no lo tenían. Solo PO y SO ya lo implementaban. La verificación bloquea la cancelación si el documento tiene hijos activos, previniendo violaciones de integridad referencial. Issues #119 y #145 cerrados.

- **Fix issue #138 - S2P-09: _validate_supplier_invoice_flags omite validación cuando CompanyParty es None (2026-07-09):** Se modificó `_validate_supplier_invoice_flags` en `cacao_accounting/compras/__init__.py` para lanzar `PostingError` cuando no se encuentra configuración de flags (`CompanyParty` es None), en lugar de retornar silenciosamente. Esto asegura que la validación de flags sea estricta. Commit `13f77a4`. Issue #138 cerrado.

- **Fix issue #147 - O2C-22: validate_submit_prerequisites no valida almacén para ítems de stock (2026-07-09):** Se agregó parámetro `require_warehouse` a `validate_submit_prerequisites()` en `document_flow/validation.py`. Cuando está habilitado, valida que todas las líneas tengan un almacén asignado antes de la aprobación del documento. Se actualizaron handlers de submit: Purchase Receipt, Delivery Note y Stock Entry requieren almacén siempre; Sales Invoice requiere almacén solo cuando `update_inventory=True`. Se agregaron 4 tests en `tests/test_validation.py`. Commit `e30c173`.
