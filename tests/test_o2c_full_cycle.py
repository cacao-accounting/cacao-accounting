...
def _ensure_item(code="ITEM-O2C", warehouse="WH-MAIN"):
    ...
    inv_account = database.session.execute(
        database.select(Accounts).filter_by(entity="cacao", account_type="inventory")
    ).scalars().first()
    if inv_account and not database.session.execute(
...
def test_sales_quotation_workflow(app_ctx):
    ...
    rel = database.session.execute(
        database.select(DocumentRelation).filter_by(source_id=sq.id, target_id=so.id)
    ).scalar_one()
    rel.status = "reverted"
