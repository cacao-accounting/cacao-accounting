...
def test_supplier_portal_full_functionality():
    ...
    with app.app_context():
        ...
        database.session.add(
            PurchaseQuotationItem(
                purchase_quotation_id=q_alfa.id, item_code="ITM-Q", qty=1, rate=Decimal("150.00"), amount=Decimal("150.00")
            )
        )
        database.session.add(
            PurchaseOrderItem(
                purchase_order_id=o_alfa.id, item_code="ITM-O", qty=1, rate=Decimal("250.00"), amount=Decimal("250.00")
            )
        )
        database.session.add(
            PurchaseReceiptItem(
                purchase_receipt_id=r_alfa.id, item_code="ITM-R", qty=1, rate=Decimal("350.00"), amount=Decimal("350.00")
            )
        )
        database.session.add(
            PurchaseInvoiceItem(
                purchase_invoice_id=note_alfa.id, item_code="ITM-N", qty=1, rate=Decimal("50.00"), amount=Decimal("50.00")
            )
        )
        database.session.add(
            PurchaseInvoiceItem(
                purchase_invoice_id=inv_alfa.id, item_code="ITM-I", qty=1, rate=Decimal("450.00"), amount=Decimal("450.00")
            )
        )
        database.session.commit()
