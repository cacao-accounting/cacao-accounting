...
Se revisó la tabla compartida de impuestos y cargos de la factura de compra.
El tratamiento contable seleccionado por el usuario es la fuente de verdad;
se eliminó el checkbox duplicado `Afecta inventario`. La propuesta fiscal del
proveedor puede precargar el formulario, pero una plantilla seleccionada por el
usuario debe conservarse en el document flow.

La regla de contabilización confirmada es:

- Un impuesto normal se contabiliza con la factura y forma parte de cuentas por
  pagar y del monto liquidado al proveedor, salvo retenciones.
- Un impuesto marcado como `capitalizable_inventory_cost` se reconoce una sola
  vez en la recepción y la factura conserva el impuesto fiscal sin volver a
  aumentar el valor del inventario.
- Los landed costs pertenecen al flujo de recepción/valoración de inventario;
  no se incorporan a la factura ni participan en la deduplicación de impuestos.

### Implementación y cobertura

- La interfaz compartida deriva `affects_inventory` exclusivamente del
  tratamiento contable y muestra una explicación cuando el impuesto es
  capitalizable.
- Persistencia, preview y snapshots ignoran valores obsoletos del checkbox.
- El flujo de factura identifica impuestos capitalizables ya reconocidos en la
  recepción mediante `source_rule_id` y evita la doble capitalización.
- La identidad de tipo se guarda en el detalle de asignación para que cargos y
  landed costs no se confundan con impuestos.
- Se agregaron pruebas JavaScript, de persistencia y de integración para
  tratamiento autoritativo y no duplicación.
- La factura de compra conserva la plantilla seleccionada en creación,
  edición y duplicación; cuando existe un documento origen, solo se usa su
  plantilla como propuesta y la selección explícita del usuario prevalece.
- El filtro de no duplicación se limita a eventos de confirmación de factura;
  los eventos de recepción e importación continúan procesando todos sus
  cargos capitalizables.

### Correcciones del feedback de revisión

- La causa de las regresiones globales fue el alcance incorrecto del filtro en
  `import_landed_cost_confirmed`; recepción e importación vuelven a procesar
  cargos, mientras que la deduplicación se limita a la confirmación de factura.
- La política `affects_inventory_from_treatment` se centralizó en
  `accounting_engine.common.fiscal`; el tratamiento contable sigue siendo la
  única fuente de verdad después de retirar el checkbox.
- Las plantillas de impuestos de compras se validan por existencia, estado
  activo, compañía, tipo de compra y moneda antes de persistirse.
- Los reversos preservan `source_rule_id` y `tax_type`; los identificadores de
  deduplicación usan namespaces `rule:` y `concept:` para evitar colisiones.
- La serialización sincroniza el tratamiento antes de construir el payload y
  el modal expone la ayuda contextual mediante `aria-describedby`.

### Corrección de los dos issues restantes del feedback

- La validación de plantillas de compras ahora exige `template_type="buying"`.
  Se verificó el modelo real: `TaxTemplate` no tiene una columna `applies_to`,
  por lo que no se agregó una referencia inexistente.
- El texto contextual del modal dejó de usar `role="status"`; permanece
  asociado al selector mediante `aria-describedby` sin anuncios intrusivos.
- Se normalizó la indentación del bloque HTML y se agregó una prueba de
  aceptación/rechazo de plantillas de compra.
