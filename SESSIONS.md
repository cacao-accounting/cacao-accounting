...
- Dentro de la iteración de entradas en Python, si la configuración original tiene `include_closing=False`, se omiten selectivamente los asientos de cierre (`entry.is_fiscal_year_closing == True`) si son de tipo P&L (`classification in _PL_CLASSIFICATIONS`) o si pertenecen al año fiscal en curso (`fiscal_year_start and entry.posting_date >= fiscal_year_start`), permitiendo conservar de forma precisa los asientos de cierre de las utilidades retenidas acumuladas de años fiscales cerrados. Cuando no hay un período/límite contable suministrado (es decir, `fiscal_year_start is None`), se preserva la exclusión completa original de los asientos de cierre para evitar la duplicación de ingresos del período.
- Se extrajo el helper privado `_should_skip_balance_sheet_entry` en `cacao_accounting/reportes/services.py` para reducir la complejidad cognitiva de `get_balance_sheet_report` y cumplir de forma impecable con las reglas de mantenimiento y cobertura de SonarCloud.
- Se agregó el caso de prueba robusto `test_multiannual_balance_sheet_balanced` en `tests/test_fiscal_year_closing.py` que crea dos años fiscales, realiza transacciones en el primero, ejecuta el cierre de año fiscal y valida que el reporte para el año siguiente tenga un cuadre perfecto (`difference == 0`). Además, se agregó `test_balance_sheet_without_period_balanced` para verificar el correcto funcionamiento cuando no se especifica un período de año fiscal.
