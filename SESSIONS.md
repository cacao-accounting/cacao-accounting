...
## 2026-08-13 — Auditoría y corrección del flujo de pagos, notas de crédito y notas de débito (AP y AR)

### Petición
Realizar una auditoría al flujo de pagos, notas de crédito y notas de débito para asegurar que los ledgers de contabilidad AP y AR sean correctos, robustos y reconciliables.

### Diagnóstico e implementación
- **Detección de Gaps en AP (Compras)**: Durante la auditoría se identificó que mientras en el módulo de ventas (AR) las notas de crédito/débito persistían correctamente las relaciones documentales de tipo `invoice_reversal` hacia las facturas origen, el módulo de compras (AP) carecía por completo de esta persistencia. Por ende, las notas de crédito de proveedor nunca afectaban el saldo outstanding calculado en el subledger de cuentas por pagar.
- **Implementación de Relaciones S2P**: Se crearon las funciones `_validate_purchase_reversal_of` y `_persist_purchase_reversal_relation` en `cacao_accounting/compras/__init__.py`. Estas validan la factura de origen (que exista, esté aprobada, pertenezca al mismo proveedor y compañía, y que el importe de la nota no exceda el saldo vivo) e insertan el registro de `DocumentRelation` correspondiente al aprobar una nota.
- **Robustecimiento en Cancelación de Notas (AP y AR)**: Se corrigió un problema donde la cancelación de una nota de crédito o débito no recalculaba el saldo outstanding de la factura origen. Tanto `compras_factura_compra_cancel` como `ventas_factura_venta_cancel` ahora actualizan de forma reactiva la cache outstanding del documento origen afectado.
- **Pruebas Unitarias de Integración**: Se creó el archivo `tests/test_s2p_purchase_notes.py` para asegurar que el flujo de notas en AP reduzca los saldos, respete los límites acumulados y actualice correctamente las caches en caso de cancelaciones de notas de crédito de compra. Todas las pruebas pasan con éxito.
