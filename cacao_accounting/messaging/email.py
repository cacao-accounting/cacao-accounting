...
def encrypt_smtp_pass(plaintext: str) -> str:
    ...
    try:
        ...
        raise EmailError(_("Error al cifrar contraseña: %(error)s") % {"error": exc}) from exc
...
def retry_email_queue_item(queue_id: str) -> Any:
    ...
    try:
        ...
        database.session.commit()
        raise EmailError(_("Error al reintentar envío: %(error)s") % {"error": exc}) from exc
...
def send_email(to_email: str, subject: str, body: str, is_html: bool = False) -> None:
    ...
    try:
        ...
        raise EmailError(_("Puerto SMTP no válido: %(port)s") % {"port": port_str}) from exc
    ...
    try:
        ...
        raise EmailError(_("Error al enviar correo electrónico: %(error)s") % {"error": exc}) from exc
