...
def validate_batch_params(params: BatchParams) -> Item:
    ...
    if item is None:
        raise InventoryServiceError(_("El item '%(item_code)s' no existe.") % {"item_code": params.item_code})
    ...
    if existing is not None:
        raise InventoryServiceError(
            _("El lote '%(batch_no)s' ya existe para el item %(item_code)s.") % {"batch_no": batch_no, "item_code": item.code}
        )
...
def update_item_with_uoms(
    ...
    item = database.session.execute(select(Item).filter_by(code=item_code)).scalar_one_or_none()
    if item is None:
        raise InventoryServiceError(_("El item '%(item_code)s' no existe.") % {"item_code": item_code})
...
def validate_item_uom_rows(default_uom: str, rows: list[ItemUOMRow]) -> None:
    ...
    for row in rows:
        ...
        if database.session.execute(select(UOM).filter_by(code=row.uom_code)).scalar_one_or_none() is None:
            raise InventoryServiceError(_("La UOM '%(uom_code)s' no existe.") % {"uom_code": row.uom_code})
...
def _validate_single_item_account_row(row: ItemAccountRow, requires_expense: bool, seen_companies: set[str]) -> None:
    ...
    if database.session.execute(select(Entity).filter_by(code=row.company)).scalar_one_or_none() is None:
        raise InventoryServiceError(_("La compañia '%(company)s' no existe.") % {"company": row.company})
...
def _validate_item_account(company: str, account_id: str | None, expected_type: str, label: str) -> None:
    ...
    if account is None or account.entity != company:
        raise InventoryServiceError(_("La cuenta de %(label)s no pertenece a la compañia seleccionada.") % {"label": label})
    ...
    if account_type not in expected_aliases:
        raise InventoryServiceError(
            _("La cuenta de %(label)s debe ser valida para %(label)s en la compañia.") % {"label": label}
        )
