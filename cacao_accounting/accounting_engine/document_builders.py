...
def _payment_build_spec(document: PaymentEntry) -> PaymentBuildSpec | None:
    ...
    if payment_type not in {"pay", "receive"}:
        return None
    party_type = _payment_party_type(document, payment_type)
    return PaymentBuildSpec(
        payment_type=payment_type,
        direction="sales" if party_type == "customer" else "purchase",
        event_type=_payment_event_type(payment_type, party_type),
...
def _payment_party_type(document: PaymentEntry, payment_type: str) -> str:
    """Resolve a payment party type, including legacy defaults."""
    party_type = (getattr(document, "party_type", None) or "").lower()
    if party_type in {"customer", "supplier"}:
        return party_type
    return "supplier" if payment_type == "pay" else "customer"


def _payment_event_type(payment_type: str, party_type: str) -> str:
    """Resolve the fiscal event generated by a payment direction."""
