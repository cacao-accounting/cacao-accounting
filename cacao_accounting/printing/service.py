...
class PrintService:
    ...
    def validate_template(
        ...
        if doc_def is None:
            raise TemplateValidationError(
                _("Document type %(document_type)s is not registered.") % {"document_type": document_type}
            )
        ...
        try:
            ...
            raise TemplateValidationError(_("Error de renderizado Jinja2: %(error)s") % {"error": exc}) from exc
    ...
    def resolve_template(
        ...
        if global_default is not None:
            return global_default
        raise PrintTemplateNotFoundError(
            _("No published template found for %(document_type)s.") % {"document_type": document_type}
        )
    ...
    def build_print_html(
        ...
        return (
            ...
            "        .print-toolbar a, .print-toolbar button { border: 1px solid #6c757d; border-radius: 4px;\n"
            "            background: #fff; color: #212529; cursor: pointer; font: inherit; padding: 6px 12px;\n"
            "            text-decoration: none; }\n"
            "        .print-toolbar button { background: #0d6efd; border-color: #0d6efd; color: #fff; }\n"
            "        .print-document { margin: 0 auto; max-width: 900px; }\n"
            "        @media print {\n"
            "            [data-print-exclude] { display: none !important; }\n"
