...
ACCOUNT_TYPE_ALLOWED_VOUCHERS: dict[str, frozenset[str]] = {
    ...
    "petty_cash": frozenset({"payment_entry", "bank_transaction", "exchange_revaluation", "journal_entry"}),
    ...
    "exchange_gain": frozenset({"exchange_revaluation", "journal_entry", "payment_entry"}),
    "exchange_loss": frozenset({"exchange_revaluation", "journal_entry", "payment_entry"}),
    "unrealized_exchange_gain": frozenset({"exchange_revaluation", "payment_entry"}),
    "unrealized_exchange_loss": frozenset({"exchange_revaluation", "payment_entry"}),
...
def load_catalog_default_mapping(catalog_file: str | Path) -> dict[str, str]:
    ...
    if not mapping_path.is_file():
        raise DefaultAccountError(_(f"El catalogo {Path(catalog_file).name} no tiene mapping JSON de cuentas."))
...
def validate_default_account_assignment(company: str, field: str, account_id: str | None) -> None:
    ...
    if account_type and account_type not in definition.allowed_account_types:
        ...
        raise DefaultAccountError(_(f"La cuenta {account.code} debe ser de tipo: {allowed}."))
...
def apply_catalog_default_mapping(company: str, catalog_file: str | Path) -> CompanyDefaultAccount:
    ...
    for field, account_code in code_mapping.items():
        ...
        if not account:
            raise DefaultAccountError(_(f"La cuenta {account_code} no existe en la compania {company}."))
...
def validate_gl_account_usage(account_id: str, voucher_type: str | None) -> None:
    ...
    if is_manual_voucher and account_type in MANUAL_BLOCKED_ACCOUNT_TYPES:
        raise DefaultAccountError(_(f"La cuenta {account.code} de tipo {account_type} no permite afectacion manual."))
    ...
    if allowed_vouchers and voucher_type not in allowed_vouchers:
        raise DefaultAccountError(
            _(f"La cuenta {account.code} de tipo {account_type} no puede afectarse desde {voucher_type or 'este origen'}.")
        )
