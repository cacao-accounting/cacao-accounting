...
def _document_contexts(document: Any, ledger_code: str | Sequence[str] | None = None) -> list[LedgerContext]:
    ...
    if document_base_currency != default_company_currency:
        raise PostingError(
            _(
                "El snapshot de base_currency (%(document_base_currency)r) no coincide con la moneda "
                "funcional vigente (%(default_company_currency)r); el documento requiere revalidacion."
            )
            % {
                "document_base_currency": document_base_currency,
                "default_company_currency": default_company_currency,
            }
        )
    ...
    for book in _active_books(company, ledger_code):
        ...
        if not book_currency:
            raise PostingError(
                _("El libro contable %(ledger_code)s requiere una moneda funcional configurada.") % {"ledger_code": book.code}
            )
...
def _account_id_for_comprobante_line(line: Any, company: str) -> str:
    ...
    if not account:
        raise PostingError(
            _("La cuenta contable '%(account_code)s' no existe para la compañía.") % {"account_code": account_code}
        )
    return account.id
...
def _entry_exchange_rate(context: LedgerContext, exchange_rate: Decimal | None) -> Decimal:
    ...
    try:
        ...
        raise PostingError(_("No se pudo determinar el tipo de cambio para multimoneda: %(error)s") % {"error": exc}) from exc
...
def _lookup_exchange_rate(origin: str, destination: str, posting_date: Any) -> Decimal:
    ...
    raise PostingError(
        _("No existe tipo de cambio registrado para %(origin)s -> %(destination)s en la fecha %(posting_date)s.")
        % {"origin": origin, "destination": destination, "posting_date": posting_date}
    )
        ...
        return [
            _create_gl_entry(
                ...
                params=GLEntryParams(
                    ...
                    debit=amount,
                    credit=Decimal("0"),
                    party_type=party_type if not debit_bank_account_id else None,
                    party_id=party_id if not debit_bank_account_id else None,
            ...
            _create_gl_entry(
                ...
                params=GLEntryParams(
                    ...
                    debit=Decimal("0"),
                    credit=amount,
                    party_type=party_type if not credit_bank_account_id else None,
                    party_id=party_id if not credit_bank_account_id else None,
...
def _line_rate(line: StockEntryItem) -> Decimal:
    ...
    if amount > 0 and qty_in_base_uom <= 0:
        raise PostingError(
            _("La linea de inventario %(item_code)s requiere cantidad para valorar el monto.") % {"item_code": line.item_code}
        )
    ...
    if rate <= 0:
        raise PostingError(
            _("La linea de inventario %(item_code)s requiere tasa de valuacion.") % {"item_code": line.item_code}
        )
    return rate
...
def _line_rate_generic(line: Any) -> Decimal:
    ...
    if amount > 0 and qty_in_base_uom <= 0:
        ...
        raise PostingError(
            _("La linea de inventario %(item_code)s requiere cantidad para valorar el monto.") % {"item_code": item_code}
        )
    if rate > 0 and raw_qty > 0 and raw_qty != qty_in_base_uom:
        rate = rate * raw_qty / qty_in_base_uom
    if amount > 0 and qty_in_base_uom > 0:
    ...
    if rate <= 0:
        item_code = getattr(line, "item_code", "desconocido")
        raise PostingError(_("La linea de inventario %(item_code)s requiere tasa de valuacion.") % {"item_code": item_code})
    return rate
...
def _consume_valuation_layer(queue: list, remaining: Decimal) -> Decimal:
    """Consume cantidad desde el frente FIFO; entradas son ``[layer_id, qty, rate]``."""
    while remaining > 0 and queue:
        available_qty = queue[0][1]
        consumed_qty = min(available_qty, remaining)
        remaining -= consumed_qty
        queue[0][1] = available_qty - consumed_qty
        if queue[0][1] <= 0:
            queue.pop(0)
    return remaining


def _consume_pinned_layer(queue: list, source_layer_id: str | None, remaining: Decimal) -> Decimal:
    """Consume primero de la capa origen fijada; el excedente sigue el orden FIFO.

    Fijar cada consumo a su capa real impide que un receipt retroactivo
    insertado despues del consumo reescriba la composicion ya publicada
    (AUDIT-004).
    """
    if source_layer_id is not None:
        for index, entry in enumerate(queue):
            if entry[0] == source_layer_id and entry[1] > 0:
                consumed_qty = min(entry[1], remaining)
                remaining -= consumed_qty
                entry[1] -= consumed_qty
                if entry[1] <= 0:
                    queue.pop(index)
                break
    if remaining > 0:
        remaining = _consume_valuation_layer(queue, remaining)
    return remaining
...
def _process_valuation_layer(
    ...
    if qty == 0 and layer_value != 0 and queue:
        total_qty = sum((available_qty for _, available_qty, _ in queue), Decimal("0"))
        if total_qty > 0:
            adjustment_rate = layer_value / total_qty
            for entry in queue:
                entry[2] += adjustment_rate
    ...
    if qty > 0:
        if negative_balance > 0:
            ...
            qty -= offset
        if qty > 0:
            queue.append([layer_id, qty, rate])
        return queue, negative_balance, True
    if qty < 0:
        negative_balance += _consume_pinned_layer(queue, source_layer_id, abs(qty))
    return queue, negative_balance, True


def _schedule_valuation_layers(layers: Sequence[Any]) -> list[tuple[tuple, Any]]:
    """Ordena capas para reconstruir la cola FIFO con composicion historica inmutable.

    Las capas de reversa (qty > 0 con ``source_layer_id``) comparten la clave de
    orden de su capa origen con secuencia posterior, de modo que la cancelacion
    devuelve cantidad y valor exactamente a la capa consumida sin alterar el
    costo FIFO de ventas posteriores (AUDIT-004).
    """
    scheduled: list[tuple[tuple, Any]] = []
    restore_sequence = 0
    for layer in layers:
        qty = _decimal_value(layer.qty)
        if qty > 0 and layer.source_layer_id:
            src_key = next(
                (key for key, candidate in scheduled if candidate.id == layer.source_layer_id),
                None,
            )
            if src_key is not None:
                restore_sequence += 1
                scheduled.append(((src_key[0], src_key[1], restore_sequence), layer))
                continue
        scheduled.append(((layer.posting_date, str(layer.id), 0), layer))
...
def _valuation_queue(
    ...
    for _ignored_layer, layer in scheduled:
        ...
        queue, negative_balance, _should_continue = _process_valuation_layer(
            queue,
            negative_balance,
            qty,
            rate,
...
def _moving_average_valuation(available: list, total_available: Decimal, quantity: Decimal) -> tuple[Decimal, Decimal]:
    total_value = sum((entry[1] * entry[2] for entry in available), Decimal("0"))
...
def _fifo_valuation(available: list, quantity: Decimal) -> tuple[Decimal, Decimal, str | None, list[dict[str, str]]]:
    ...
    while remaining > 0 and queue:
        ...
        total_cost += consume_qty * entry[2]
...
def _consume_stock_valuation_layers(
    ...
    if quantity <= 0:
        raise PostingError(
            _("La cantidad de consumo debe ser mayor que cero para el artículo %(item_code)s en la bodega %(warehouse)s.")
    ...
    if total_available < quantity:
        raise PostingError(
            _(
                "No hay suficiente inventario para calcular el costo real para el artículo %(item_code)s "
                "en la bodega %(warehouse)s."
            )
            % {"item_code": item_code, "warehouse": warehouse}
        )
        ...
        database.session.add(
            StockValuationLayer(
                **layer_kwargs,
                ...
                source_layer_id=getattr(line, "_inventory_source_layer_id", None),
            )
        )
...
def _validate_stock_entry_warehouses(document: StockEntry, line: StockEntryItem) -> None:
    ...
    for warehouse_code in filter(None, warehouse_codes):
        ...
        if not warehouse or warehouse.company != document.company:
            raise PostingError(
                _("La bodega %(warehouse_code)s no pertenece a la compañía %(company)s.")
                % {"warehouse_code": warehouse_code, "company": document.company}
            )
        if not warehouse.is_active:
            raise PostingError(_("La bodega %(warehouse_code)s está inactiva.") % {"warehouse_code": warehouse_code})
...
def _delivery_already_returned_qty(
    ...
    return database.session.execute(
        ...
        .where(
            DeliveryNote.company == document.company,
            DeliveryNote.is_return.is_(True),
            DeliveryNote.reversal_of == source.id,
...
def _purchase_return_cost(document: Any, line: Any, warehouse: str, quantity: Decimal) -> tuple[Decimal, str | None]:
    ...
    if total_available < quantity:
        raise PostingError(
            _("La devolución (%(quantity)s) excede la cantidad disponible en la recepción origen (%(total_available)s).")
            % {"quantity": quantity, "total_available": total_available}
        )
    remaining_to_skip = _decimal_value(already_returned)
    remaining = quantity
    cost = Decimal("0")
...
def _restored_source_layer_id(movement: StockLedgerEntry) -> str | None:
    """Resuelve la capa origen a la que una reversa debe devolver cantidad/valor.

    Busca en las capas de consumo (qty < 0) del voucher revertido su capa de
    ingreso fijada (``source_layer_id``). Solo se fija la reversa cuando el
    consumo proviene de una unica capa; con multiples capas la composicion
    mixta no puede restaurarse con una sola capa y se usa el legado.
    """
    source_ids = (
        database.session.execute(
            select(StockValuationLayer.source_layer_id)
            .where(
                ...
                StockValuationLayer.qty < 0,
                StockValuationLayer.source_layer_id.is_not(None),
            )
            .distinct()
        )
        .scalars()
...
def _validate_stock_reversal_capacity(movements: Sequence[StockLedgerEntry]) -> None:
    ...
    for movement in movements:
        ...
        if key not in projected:
            ...
            if item is None:
                raise PostingError(_("El artículo %(item_code)s no existe.") % {"item_code": movement.item_code})
            items[key] = item
        projected[key] -= _decimal_value(movement.qty_change)
        if projected[key] < 0 and not items[key].allow_negative_stock:
            raise PostingError(
                _(
                    "No se puede cancelar el movimiento de %(item_name)s: "
                    "la bodega %(warehouse)s quedaría con stock negativo."
                )
                % {"item_name": items[key].name, "warehouse": movement.warehouse}
            )
...
def _persisted_outbound_cost(document: Any, line: Any) -> Decimal | None:
    """Costo de salida del mayor de inventario persistido para la linea.

    AUDIT-004: el GL de salidas no debe depender del atributo transitorio de
    sesion ``_inventory_cost_amount``; resuelve el monto desde el
    StockLedgerEntry publicado (columna persistida) cuando el atributo no esta
    disponible. Devuelve None si el voucher no tiene salidas registradas.
    """
    voucher_type = _get_voucher_type(document)
    voucher_id = _get_voucher_id(document)
    item_code = getattr(line, "item_code", None)
    if not item_code or not voucher_id:
        return None
    warehouse = (
        getattr(line, "warehouse", None)
    ...
    query = select(func.coalesce(func.sum(-StockLedgerEntry.stock_value_difference), 0)).where(
        StockLedgerEntry.company == document.company,
        StockLedgerEntry.voucher_type == voucher_type,
        StockLedgerEntry.voucher_id == voucher_id,
        StockLedgerEntry.item_code == item_code,
    ...
    if warehouse:
        query = query.where(StockLedgerEntry.warehouse == warehouse)
    total = _decimal_value(database.session.execute(query).scalar_one())
...
def _get_delivery_note_line_value(document: DeliveryNote, line: Any) -> Decimal:
    ...
    if cost_amount is None:
        cost_amount = _persisted_outbound_cost(document, line)
...
def _add_stock_entry_line_gl_entries(
    ...
    amount = _get_stock_entry_line_amount(document, line, purpose)
    if amount <= 0:
        return
...
def _get_stock_entry_line_amount(document: StockEntry, line: StockEntryItem, purpose: str) -> Decimal:
    ...
    cost_amount = getattr(line, "_inventory_cost_amount", None)
    if cost_amount is None:
        # AUDIT-004: resolver desde el mayor persistido cuando la sesion de
        # posting no sembro el atributo transitorio de costo de salida.
        cost_amount = _persisted_outbound_cost(document, line)
...
def _validate_stock_reconciliation_dimensions(document: StockEntry, company: str) -> None:
    ...
    for model, code, label in dimensions:
        ...
        if exists is None:
            raise PostingError(
                _("El %(label)s de la conciliación no pertenece a la compañía del documento.") % {"label": label}
            )
