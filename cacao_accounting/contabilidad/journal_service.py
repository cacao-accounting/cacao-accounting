...
def duplicate_journal_as_reversal_draft(
    ...
    # NOTA: La validación de periodo distinto es INTENCIONAL y CORRECTA (NO es un issue).
    # R2R-04 (ISSUE #174) fue verificado como FALSO POSITIVO por auditoría independiente.
...
def _validate_line_books(company: str, books: list[str] | None, lines: list[JournalLineInput]) -> None:
    ...
    if invalid:
        raise JournalValidationError(
            _("Libro contable no válido para la compañía: %(ledger_code)s.") % {"ledger_code": sorted(invalid)[0]}
        )
