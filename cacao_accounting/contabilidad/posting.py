...
def _get_stock_entry_line_amount(line: StockEntryItem, purpose: str) -> Decimal:
    ...
    # Fetch actual stock value difference from StockLedgerEntry to ensure GL is synced with Kardex
    sle = (
        database.session.execute(
            select(StockLedgerEntry).filter_by(
                voucher_type="stock_entry",
                voucher_id=str(line.stock_entry_id),
                item_code=line.item_code,
            )
        )
        .scalars()
    ...
    if sle:
        return abs(_decimal_value(sle.stock_value_difference))
