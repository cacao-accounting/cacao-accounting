...
La primitiva ``validate_active_currency`` exige que una moneda exista y
este activa antes de usarse en un par de conversion.
"""
...
class CurrencyGuard:
    ...
    def get_currency(self, code: str | None) -> Currency:
        ...
        if not code:
            raise CurrencyGuardError(_("La moneda es obligatoria."))
        currency = database.session.execute(database.select(Currency).filter_by(code=code)).scalar_one_or_none()
        if currency is None:
            raise CurrencyGuardError(_("La moneda indicada no existe."))
        return currency
    ...
    def assert_can_deactivate(self, currency: Currency) -> None:
        ...
        if bool(currency.default):
            raise CurrencyGuardError(_("No se puede deshabilitar la moneda predeterminada del sistema."))
        ...
        if active_book is not None:
            raise CurrencyGuardError(_("No se puede deshabilitar la moneda porque esta en uso por un libro contable activo."))
    ...
    def apply_currency_edit(self, currency: Currency, *, active: bool, default: bool) -> None:
        ...
        if default and not active:
            raise CurrencyGuardError(_("La moneda predeterminada del sistema debe permanecer activa."))
        if not active:
